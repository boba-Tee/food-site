...
export default function CartProvider({children}) {
    ...
    const [totalPrice, setTotalPrice] = useState(initCart.totalPrice); // 100 is placeholder for now
    const [totalCount, setTotalCount] = useState(initCart.totalCount); // 10 is placeholder for now
    ...
    const addToCart = food => {
        ...
        // if we have the cartItem already in our cart we change quantity instead of duplicating the cartItem in cart
        if (cartItem) { // if cartItem is not null/undefined
            changeQuantity(cartItem, cartItem.quantity + 1); // increase cartItem count by 1
        } else { // otherwise, item does not exist in our cart
            // update cart items with array of previous cartItems, and the new input food w/ quantity of 1 and it's food.price
    ...
    const clearCart = () => {
        ...
        // get the values we defined in EMPTY_CART
    ...
    return (
        // CartContext.Provider that provides the cart value for it's children
